...
char *GetHistoryFile(info_t *info)
{
	...
	if (!dir)
		return (NULL);
	buf = malloc(sizeof(char) * (StrLenFunction(dir) + StrLenFunction(HistoryFile) + 2));
	if (!buf)
		return (NULL);
	buf[0] = 0;
	StrCpyFunction(buf, dir);
	StrCatFunction(buf, "/");
	StrCatFunction(buf, HistoryFile);
	return (buf);
}
...
int WriteHistory(info_t *info)
{
	...
	{
		PutsFdFunction(node->str, fd);
		PutFdFunction('\n', fd);
	}
	PutFdFunction(BufferFlush, fd);
	close(fd);
	return (1);
}
...
int ReadHistory(info_t *info)
{
	...
	while (info->histcount-- >= HistoryMax)
