...
ssize_t GetInput(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buffer */
	{
		...
		if (i >= len) /* reached end of buffer? */
		{
			...
			info->CMD_Buf_type = CommandNormal;
		}

		*buf_p = p; /* pass back pointer to current command position */
		return (StrLenFunction(p)); /* return length of current command */
	}
...
ssize_t read_buf(info_t *info, char *buf, size_t *i)
{
	...
	if (*i)
		return (0);
	r = read(info->readfd, buf, ReadBufferSize);
	if (r >= 0)
		*i = r;
	return (r);
}
...
int GetLine(info_t *info, char **ptr, size_t *length)
{
	...
	new_p = ReallocFunction(p, s, s ? s + k : k + 1);
	...
	if (s)
		...
		StrNCpyFunction(new_p, buf + i, k - i + 1);
