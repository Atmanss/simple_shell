...
list_t *AddNode(list_t **head, const char *str, int num)
{
	...
	if (!new_head)
		return (NULL);
	MemSetFunction((void *)new_head, 0, sizeof(list_t));
	new_head->num = num;
	if (str)
	{
		new_head->str = StrDupFunction(str);
		if (!new_head->str)
		{
			free(new_head);
			return (NULL);
...
list_t *AddNodeEnd(list_t **head, const char *str, int num)
{
	...
	if (!new_node)
		return (NULL);
	MemSetFunction((void *)new_node, 0, sizeof(list_t));
	new_node->num = num;
	if (str)
	{
		new_node->str = StrDupFunction(str);
		if (!new_node->str)
		{
			free(new_node);
			return (NULL);
